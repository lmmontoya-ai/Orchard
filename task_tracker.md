...
- The first real-SSD request-shape rerun against `.git\objects` changed the optimization target again: the copied subtree is about `9.3 GiB`, `mounted.read_requests.large=9591`, `mounted.read_requests.sequential=9017`, and both `read_ahead_hits` and `read_ahead_prefetches` were still zero. That means the remaining gap is large sequential streaming, not small-file churn or metadata reloads. `src/fs-winfsp/src/mount.cpp` now keeps sequential read state moving forward even when a request is served from cached read-ahead bytes, and large streaming files can now grow their prefetch window beyond the current request size (up to `4 MiB`) instead of stalling at the older `1 MiB` cap. Focused regression coverage is `WinFspMountedVolumeReadAheadPrefetchesLargeStreamingReads`, which mounts a synthetic multi-megabyte APFS fixture and verifies chained prefetches plus at least one read-ahead hit on large sequential reads while preserving `WinFspMountedVolumeDoesNotReadAheadSmallSequentialReads`. The remaining follow-up is another short real-SSD `.git\objects` rerun with `--diagnose-perf` to confirm `read_ahead_prefetches` and `read_ahead_hits` are no longer zero and that wall-clock time drops materially.
